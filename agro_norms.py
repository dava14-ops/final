...
# ─── Поправочные коэффициенты (Таблица 1.1, стр. 9) ────────────────
# 1.1а: Каменистость
K_STONINESS: Dict[str, Dict[str, float]] = {
    ...
    "кошение трав": {"отсутствует": 1.00, "слабая": 0.90, "средняя": 0.85, "сильная": 0.00},
}

# 1.1б: Высота над уровнем моря
K_ALTITUDE: Dict[str, Dict[str, float]] = {
    "пахотные":   {"до 500": 1.00, "500-1000": 0.95, "1000-1500": 0.91, "1500-2000": 0.88},
    "непахотные": {"до 500": 1.00, "500-1000": 0.98, "1000-1500": 0.96, "1500-2000": 0.94},
}

# 1.1в: Сложность конфигурации полей (группа I–V)
K_FIELD_CONFIG: Dict[str, Dict[str, float]] = {
    "пахотные":   {"I": 1.00, "II": 0.97, "III": 0.91, "IV": 0.81, "V": 0.75},
    "непахотные": {"I": 1.00, "II": 0.96, "III": 0.92, "IV": 0.83, "V": 0.79},
}

# 1.1г: Изрезанность полей препятствиями (% площади)
K_OBSTACLES: Dict[str, Dict[str, float]] = {
    "пахотные": {
        "0": 1.00, "до 5": 0.96, "5-10": 0.92, "10-15": 0.88,
        "15-20": 0.84, "20-25": 0.81, "25-30": 0.77, "30-35": 0.74,
    },
    "непахотные": {
        "0": 1.00, "до 5": 0.96, "5-10": 0.93, "10-15": 0.89,
        "15-20": 0.86, "20-25": 0.83, "25-30": 0.80, "30-35": 0.77,
    },
}

# 1.1д: Рельеф (угол склона)
K_RELIEF: Dict[str, Dict[str, float]] = {
...
def calculate_k_ob(
    ...
    """
    Вычисляет обобщённый поправочный коэффициент времени смены K_об.
    
    Формула (Сроки.ПДФ, формула 1.2):
        K_об = K_K · K_h · K_C · K_П · K_R
    
    Parameters
    ----------
    operation_type : str
        Тип работ: "пахотные", "непахотные", "кошение трав".
    stoniness : str
        Степень каменистости: "отсутствует", "слабая", "средняя", "сильная".
    altitude : str
        Высота над уровнем моря: "до 500", "500-1000", "1000-1500", "1500-2000".
    field_config : str
        Группа сложности конфигурации полей: "I", "II", "III", "IV", "V".
    obstacles_pct : str
        Площадь, занимаемая препятствиями (%): "0", "до 5", "5-10", ..., "30-35".
    relief_slope : str
        Угол склона: "<=1", "1-3".
    ...
    -------
    float
        K_об ∈ (0, 1]. Чем хуже условия, тем меньше K_об,
        тем больше моточасов требуется.
    """
    op_type = str(operation_type).strip().lower()
    if op_type not in K_STONINESS:
        op_type = "пахотные"
    
    k_k = K_STONINESS.get(op_type, {}).get(stoniness, 1.0)
    k_h = K_ALTITUDE.get(op_type, {}).get(altitude, 1.0)
    k_c = K_FIELD_CONFIG.get(op_type, {}).get(field_config, 1.0)
    k_p = K_OBSTACLES.get(op_type, {}).get(obstacles_pct, 1.0)
    k_r = K_RELIEF.get(op_type, {}).get(relief_slope, 1.0)
    
    result = k_k * k_h * k_c * k_p * k_r
    
    # Защита от нуля (кошение трав + сильная каменистость)
    if result <= 0.0:
        result = 0.50
