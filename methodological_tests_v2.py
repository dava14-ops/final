...
def _safe_generate_data(
    *,
    ...
    Where supported by the existing generate_data() signature, pass
    n_clusters explicitly. Otherwise generate_data() is called without it
    and the actual cluster count is measured after generation.
    """
    ...
    try:
        ...
        if "n_clusters" in sig.parameters:
            ...
            kwargs["G"] = cfg.n_clusters
        elif "clusters" in sig.parameters:
            kwargs["clusters"] = cfg.n_clusters
    except Exception:
        pass
    ...
    if "cluster_id" in data.columns:
        ...
        if cfg.n_clusters is not None and actual_g != cfg.n_clusters:
            logger.warning(
                "Запрошено G=%d, фактически в data G=%d. "
                "Проверьте сигнатуру/реализацию generate_data().",
                cfg.n_clusters,
                actual_g,
            )
...
def run_comparison(
    *,
    ...
    print(
        ...
        f"n={cfg.n}, G={cfg.n_clusters}, pi_Z={cfg.instrument_strength}"
    )
    print(f"R={n_runs}")
    print("-" * 84)
